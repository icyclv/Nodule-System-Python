...
//    AT_CHECK(overlap->is_contiguous(), "argument#3(overlap) must be contiguous");
    ...
    // Number of ROIs
    int64_t boxes1_num = boxes1->sizes()[0];
    int64_t boxes1_dim = boxes1->sizes()[1];

    int64_t boxes2_num = boxes2->sizes()[0];
    int64_t boxes2_dim = boxes2->sizes()[1];

    int64_t overlap_num = overlap->sizes()[0];
    int64_t overlap_dim = overlap->sizes()[1];
