...
from net.sanet.config import net_config as config
import copy
...
from net.sanet import resnet, cgnl
...
class CropRoi(nn.Module):
    ...
    def forward(self, f, inputs, proposals):
        ...
        for p in proposals:
            ...
            fe1 = comb2[int(b), :, int(z_start / 4):int(z_end / 4), int(y_start / 4):int(y_end / 4),
                  int(x_start / 4):int(x_end / 4)].unsqueeze(0)
            fe1_up = self.up2(fe1)

            fe2 = self.back2(torch.cat((fe1_up, out1[int(b), :, int(z_start / 2):int(z_end / 2),
                                                int(y_start / 2):int(y_end / 2),
                                                int(x_start / 2):int(x_end / 2)].unsqueeze(0)), 1))
            # fe2_up = self.up3(fe2)
