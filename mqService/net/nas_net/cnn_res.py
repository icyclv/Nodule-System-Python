...
from net.nas_net.net_sphere import *
...
class CombineModel(nn.Module):
    def __init__(self):
        ...
        self.model_1 = ConvRes([[4, 4], [4, 8], [8, 8]])
        self.model_2 = ConvRes([[8], [8, 16, 32], [32, 32, 32, 64, 64]])
        self.model_3 = ConvRes([[4, 16, 16], [16, 16, 64], [64, 64, 64]])
        self.model_4 = ConvRes([[4], [8, 16, 16], [32, 128]])
        self.model_5 = ConvRes([[8], [8, 8, 16], [16, 16, 16, 16]])
        self.model_6 = ConvRes([[4, 4], [8, 8, 8, 8], [8, 16]])
        self.model_7 = ConvRes([[8], [8, 16, 32, 32], [32]])
        self.model_8 = ConvRes([[8, 16, 16, 16], [16, 16, 16, 64], [128]])
        self.model_9 = ConvRes([[8, 16, 16, 16], [16, 16, 16, 64], [128]])

    def forward(self, inputs):
        _, out1 = torch.max(self.model_1(inputs)[0].data,1)
        _, out2 = torch.max(self.model_2(inputs)[0].data,1)
        _, out3 = torch.max(self.model_3(inputs)[0].data,1)
        _, out4 = torch.max(self.model_4(inputs)[0].data,1)
        _, out5 = torch.max(self.model_5(inputs)[0].data,1)
        _, out6 = torch.max(self.model_6(inputs)[0].data,1)
        _, out7 = torch.max(self.model_7(inputs)[0].data,1)
        _, out8 = torch.max(self.model_8(inputs)[0].data,1)
        _, out9 = torch.max(self.model_9(inputs)[0].data,1)
        out = (out1+out2+out3+out4+out5+out6+out7+out8+out9)/9
        return out
